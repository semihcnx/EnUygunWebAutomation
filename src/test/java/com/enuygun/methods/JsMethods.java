...
public class JsMethods {
    ...
    public void waitPageLoadComplete() {
        ...
        ExpectedCondition<Boolean> expectation = driver -> jsDriver
                .executeAsyncScript("return document.readyState;").toString().equals("complete");
        try {
    ...
    public void waitForAngularLoad() {
        ...
        WebDriverWait webDriverWait = new WebDriverWait(driver,10);
        ExpectedCondition<Boolean> expectation = driver -> jsDriver.executeAsyncScript(
    ...
    /**
     ...
     var select = document.querySelector("settings-ui").shadowRoot.querySelector("settings-main#main").shadowRoot.querySelector("settings-basic-page").shadowRoot.querySelector("settings-appearance-page").shadowRoot.querySelector("#zoomLevel");for(var i = 0; i < select.options.length; i++){
     if(select.options[i].value == '1')
     { select.options[i].selected = true; } }var evt = document.createEvent("HTMLEvents");
     evt.initEvent("change", true, true);select.dispatchEvent(evt);
     */
}
